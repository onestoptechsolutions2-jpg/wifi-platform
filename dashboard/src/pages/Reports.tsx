...
export default function Reports() {
  ...
  useEffect(() => {
    ...
    api.get(`/analytics/trends?days=${days}`)
      .then(r => {
        setTrends(r.data)
        // Aggregate login methods from trends
    ...
    <div className="page">
      ...
      {loading ? <span className="spinner-sm" /> : (
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem', alignItems: 'start' }}>
          <div className="card">
            <h2 style={{ fontWeight: 600, marginBottom: '1.25rem' }}>Daily Logins</h2>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={trends}>
                ...
                <Tooltip />
                <Bar dataKey="logins" fill="#1B5FAD" radius={[4,4,0,0]} name="Logins" />
          ...
          <div className="card">
            ...
            {methods.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  <Pie data={methods} dataKey="value" nameKey="name" cx="50%" cy="45%" outerRadius={90} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
